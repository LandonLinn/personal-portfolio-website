...
// ../node_modules/emailjs/dist/address.js
  ...
  if (isGroup) {
    ...
    if (addresses.length === 0 && texts.length > 0) {
      ...
      if (addresses.length === 0) {
        for (let i = texts.length - 1; i >= 0; i--) {
          texts[i] = texts[i].replace(/\s*\b[^@\s]+@[^@\s]+\b\s*/, (address2) => {
            if (addresses.length === 0) {
              addresses = [address2.trim()];
              return " ";
            } else {
              return address2;
            }
...
// ../node_modules/emailjs/dist/message.js
var import_fs = __toESM(require_fs(), 1);
var import_os = __toESM(require_os(), 1);
var import_stream = __toESM(require_stream(), 1);

// ../node_modules/emailjs/dist/date.js
...
// ../node_modules/emailjs/dist/mime.js
...
function splitMimeEncodedString(str, maxlen = 12) {
  ...
  while (str.length) {
    ...
    while (!done) {
      ...
      const match2 = str.slice(curLine.length).match(/^=([0-9A-F]{2})/i);
      if (match2) {
  ...
  checkValidity() {
    ...
    if (this.attachments.length > 0) {
      ...
      return {
        ...
        validationError: failed.length > 0 ? failed.join(", ") : void 0
      };
    }
...
var MessageStream = class extends import_stream.Stream {
  ...
  constructor(message) {
    ...
    const outputData = (attachment, callback) => {
      outputBase64(attachment.encoded ? attachment.data ?? "" : Buffer.from(attachment.data ?? "").toString("base64"), callback);
    };
    ...
    const outputRelated = (message2, callback) => {
      ...
      outputAttachment(message2, () => {
        outputMessage(boundary, message2.related ?? [], 0, () => {
          output(`${CRLF}--${boundary}--${CRLF}${CRLF}`);
          callback();
        });
      });
...
// ../node_modules/emailjs/dist/connection.js
...
// ../node_modules/emailjs/dist/error.js
var SMTPErrorStates = {
  COULDNOTCONNECT: 1,
  BADRESPONSE: 2,
  AUTHFAILED: 3,
...
// ../node_modules/emailjs/dist/response.js
var CRLF2 = "\r\n";
var SMTPResponseMonitor = class {
  ...
  constructor(stream, timeout, onerror) {
    let buffer = "";
    const notify = () => {
      const lines = buffer.split(CRLF2);
      let processedChars = 0;
      let responseFound = false;
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (!line.length && i < lines.length - 1) {
          processedChars += line.length + CRLF2.length;
          continue;
        }
        ...
        if (match) {
          ...
          if (separator === " ") {
            if (i === lines.length - 1 && !buffer.endsWith(CRLF2)) {
              processedChars += line.length + CRLF2.length;
              continue;
            }
            stream.emit("response", null, {
              ...
              data: buffer.substring(0, processedChars + line.length + CRLF2.length).replace(/\r/g, "")
              // Send the full data for this response
            });
            responseFound = true;
            processedChars += line.length + CRLF2.length;
            break;
...
var SMTPConnection = class extends import_events.EventEmitter {
  ...
  features = null;
  monitor = null;
  domain = (0, import_os2.hostname)();
  host = "localhost";
  ssl = false;
  tls = false;
  port;
  greylistResponseTracker = /* @__PURE__ */ new WeakSet();
  ...
  connect(callback, port = this.port, host = this.host, options = {}) {
    ...
    const connected = () => {
      ...
      if (this.ssl && !this.tls) {
        if (typeof this.ssl !== "boolean" && this.sock instanceof import_tls.TLSSocket && !this.sock.authorized && // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  login(callback, user, password, options = {}) {
    ...
    const initiate = (err, data) => {
      ...
      const failed = (err2, data2) => {
        this.loggedin = false;
        this.close();
        err2.message = err2.message.replace(login.password(), "REDACTED");
        caller(callback, SMTPError.create("authorization.failed", SMTPErrorStates.AUTHFAILED, err2, data2));
      };
      const response = (err2, data2, message) => {
        if (err2) {
          failed(err2, data2);
        } else {
          this.loggedin = true;
          caller(callback, err2, data2, message);
        }
      };
...
// ../node_modules/emailjs/dist/client.js
var SMTPClient = class {
  ...
  _isAttachmentInlinedHtml(attachment) {
    return attachment && (attachment.data || attachment.path) && attachment.alternative === true || false;
  }
